...
export default withMermaid(withPwa(
  defineConfig({
    ...
    pwa: {
      ...
      manifest: {
        name: 'Vilinko Studio 文档',
        short_name: 'Vilinko Docs',
        description: 'Vilinko Studio 文档，提供产品的使用说明和用户服务支持。',
        start_url: '/',
        display: 'standalone',
      ...
      workbox: {
        globPatterns: ['**/*.{js,css,html,svg,png,ico,md}'],
        runtimeCaching: [
          {
            urlPattern: ({ url }) => url.origin === 'https://www.vilinko.com',
            handler: 'CacheFirst',
            options: {
              cacheName: 'vilinko-assets',
              expiration: {
                maxEntries: 50,
